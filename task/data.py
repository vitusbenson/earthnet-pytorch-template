...
class EarthNet2021DataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: str = None):
        ...
        if stage == 'fit' or stage is None:
            earthnet_corpus = EarthNet2021Dataset(self.base_dir/"train", noisy_masked_pixels = self.hparams.noisy_masked_pixels, use_meso_static_as_dynamic = self.hparams.use_meso_static_as_dynamic, fp16 = self.hparams.fp16)
        ...
        if stage == 'test' or stage is None:
            self.earthnet_test = EarthNet2021Dataset(self.base_dir/self.__TRACKS__[self.hparams.test_track], noisy_masked_pixels = self.hparams.noisy_masked_pixels, use_meso_static_as_dynamic = self.hparams.use_meso_static_as_dynamic, fp16 = self.hparams.fp16)
